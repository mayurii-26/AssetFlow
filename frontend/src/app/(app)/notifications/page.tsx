...
import { Bell, AlertTriangle, CheckCircle, Calendar, Wrench, ClipboardCheck, Package, Check, Archive, Trash2, Filter } from "lucide-react";
import PageHeader from "@/components/ui/PageHeader";
import { mockNotifications, mockActivityLogs } from "@/lib/mock-data-extra";
      ...
      <div className="grid grid-cols-1 xl:grid-cols-3 gap-6">
          ...
          <div className="space-y-2">
            <AnimatePresence>
              {filtered.map((notif, i) => (
                <motion.div
                  ...
                  className={`flex gap-4 p-4 rounded-2xl border transition-all ${notif.read ? "bg-white/2 border-white/6 opacity-70" : `${typeBg[notif.type]}`}`}
                >
                  <div className={`w-8 h-8 rounded-xl flex items-center justify-center shrink-0 ${notif.read ? "bg-white/5" : typeBg[notif.type]}`}>
            ...
            {filtered.length === 0 && (
              <div className="py-16 text-center">
                ...
                <p className="text-[#8e9192] text-[14px]">No notifications in this category</p>
              </div>
            )}
          </div>
        </div>
        ...
        <div className="space-y-3">
          <div className="flex items-center justify-between">
            ...
            <button className="flex items-center gap-1 text-[11px] text-[#8e9192] hover:text-[#e5e2e1] transition-colors">
